...
- **Never include `.workbuddy/` directory in commit analysis or diff.** This directory is for local WorkBuddy configuration only and should not be committed to remote repositories. Always exclude it from `git status`, `git diff`, and commit messages.
